...
import { pgTable, text, varchar, integer, boolean, uuid, AnyPgColumn } from 'drizzle-orm/pg-core';
...
export const card = pgTable('card', {
    id: uuid('id').defaultRandom().primaryKey(),
    listId: uuid("listId").notNull().references(() => list.id, { onDelete: 'cascade' }),
...
export const assignedLabel = pgTable('assignedLabel', {
    cardId: uuid("cardId").notNull().references(() => card.id, { onDelete: 'cascade' }),
    labelId: uuid("labelId").notNull().references(() => label.id, { onDelete: 'cascade' }),
});
...
export const checklistItem = pgTable('checklistItem', {
    id: uuid('id').defaultRandom().primaryKey(),
    checklistId: uuid("checklistId").notNull().references(() => checklist.id, { onDelete: 'cascade' }),
